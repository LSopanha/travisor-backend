...
class AdminSV extends BaseService
{
   ...
   public function getAllAdmins() 
   {
      $query = $this->getQuery();
      if (isset($query)) {
          $admins = $query->with('role')->where('role_id', 1)->get();
          return $admins;
      } else {
   ...
   // Create new admin
   public function createNewAdmin($params) 
   {
      $admin = null;
      if (isset($params)) {
         $admin = User::create([
   ...
   // Show specific admin
   public function getAdminByGlobalId($global_id)
   {
   ...
   public function updateAdmin($params, $global_id)
   {
      $admin = User::where('users.global_id', $global_id)->first();
   ...
   public function deactivateAdmin($global_id, $active)
   {
       $admin = User::where('users.global_id', $global_id)->first();
       if (isset($admin)) {
           $admin->update([

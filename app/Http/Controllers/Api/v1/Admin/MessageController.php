...
class MessageController extends BaseAPI
{
    ...
    {
        try {
            $messages = $this->messageSV->getAllMessages();
            return $this->successResponse($messages, 'Get all messages successfully.');
        } catch(\Exception $e){
    ...
    {
        try {
            $message = $this->messageSV->getMessageByGlobalId($global_id);
            return $this->successResponse($message, 'Get message successfully.');
